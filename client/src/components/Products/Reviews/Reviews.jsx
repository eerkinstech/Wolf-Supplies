...
const Reviews = ({
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      // Call parent's submit handler - include user's name/email when available
...
toast.error(err.message || 'Failed to submit review');
  ...
  // Filter reviews based on approval requirement setting
      ...
      <div className="border-t pt-8">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={() => {
              if (!token) {
                toast('Please log in to leave a review', { icon: '🔒' });
                return;
              }
        ...
        {showReviewForm && (
          <form onSubmit={handleSubmitReview} className="bg-gray-50 p-6 rounded-lg mb-6">
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  value={user?.name || ''}
                  disabled
                  className="w-full mt-2 p-3 border rounded-lg bg-white text-gray-700"
                />
              </div>
              {/* User Email */}
              <div>
            ...
            <div className="text-right">
              <button
                type="submit"
                className="py-3 px-6 bg-black text-white rounded-lg font-semibold hover:bg-black transition"
              >
                Submit Review
              </button>
            </div>
