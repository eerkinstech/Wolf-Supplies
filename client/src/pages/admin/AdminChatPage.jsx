    ...
    const fetchConversations = async () => {
        try {
            ...
            const chatConversationsOnly = (data.data || []).filter(conv => {
                ...
                // Count message types
                const contactFormMessages = messages.filter(msg =>
                    msg.subject && msg.subject !== 'Chat Support'
                ).length;
                ...
                // LOGIC:
                // 1. If explicitly marked as chat button → Chat Tab
                if (conv.fromContactForm === false) return true;

                // 2. If has BOTH contact form AND chat messages → Show in Chat (merged)
                if (contactFormMessages > 0 && chatMessages > 0) return true;

                // 3. If only has chat messages (no contact form) → Chat Tab
                if (chatMessages > 0 && contactFormMessages === 0) return true;
    ...
    const fetchContactSubmissions = async () => {
        try {
            ...
            const contactFormOnly = (data.data || []).filter(submission => {
                ...
                // LOGIC: Show in Contact Tab if:
                // Has at least ONE contact form message (regardless of chat messages added later)
                if (contactFormMessages > 0) return true;

                // NOT chat-button-only conversations
                                            ...
                                            <div key={contact._id} className="border rounded-lg p-4" style={{ borderColor: contact.movedToChat ? 'var(--color-success)' : 'var(--color-border-light)', backgroundColor: 'var(--color-bg-section)' }}>
                                                ...
                                                <div className="mb-3 p-3 rounded" style={{ backgroundColor: 'var(--color-bg-primary)' }}>
                                                    <p className="text-sm mb-2 font-semibold" style={{ color: 'var(--color-text-primary)' }}>
                                                        {contact.messages?.[0]?.subject || contact.messages?.[0]?.message?.substring(0, 50) || 'Message'}
                                                    </p>
                                                    ...
                                                    {newsletters.map((newsletter) => (
                                                        <tr key={newsletter._id} style={{ borderColor: 'var(--color-border-light)', borderBottomWidth: '1px' }}>
                                                            ...
                                                            <td className="py-3 px-4 text-sm" style={{ color: 'var(--color-text-light)' }}>
                                                                {new Date(newsletter.createdAt || newsletter.subscribedAt).toLocaleDateString()}
                                                            </td>
                                                            <td className="py-3 px-4">
                                                                <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{
                                                                    backgroundColor: newsletter.isSubscribed ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                                                    color: newsletter.isSubscribed ? '#22c55e' : '#ef4444'
                                                                }}>
                                                                    {newsletter.isSubscribed ? 'Active' : 'Unsubscribed'}
                                                                </span>
                                                            </td>
                                                        </tr>
                                                    ))}
